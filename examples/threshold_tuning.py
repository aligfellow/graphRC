...
def write_detailed_results(f, threshold: float, results: List[Dict], stats: Dict):
    ...
    for result in results:
        if result["overall_pass"]:
            ...
            if result["detected"]:
                bonds_str = ", ".join(format_bond(b, result["symbols"]) for b in sorted(result["detected"]))
            ...
            if result["expected_dihedrals"]:  # Only show if dihedrals are expected
                summary_parts.append(
                    f"Dihedrals: Expected={n_exp_dih}, Detected={n_det_dih}, Missing={n_miss_dih}, Extra={n_extra_dih}"
                )
            ...
            # Show bond details
            if result["expected"]:
                bonds_str = ", ".join(format_bond(b, result["symbols"]) for b in sorted(result["expected"]))
                f.write(f"      Expected Bonds: {bonds_str}\n")
            if result["detected"]:
                bonds_str = ", ".join(format_bond(b, result["symbols"]) for b in sorted(result["detected"]))
                f.write(f"      Detected Bonds: {bonds_str}\n")
            if result["missing"]:
                bonds_str = ", ".join(format_bond(b, result["symbols"]) for b in sorted(result["missing"]))
                f.write(f"      Missing Bonds: {bonds_str}\n")
            if result["extra"]:
                bonds_str = ", ".join(format_bond(b, result["symbols"]) for b in sorted(result["extra"]))
...
def run_threshold_validation(output_file: str | None = None):
    ...
    basenames = [f.stem.replace(".v000", "") for f in xyz_files if f.stem.replace(".v000", "") in EXPECTED_RESULTS]
    ...
    with open(output_path, "w") as f:
        ...
        for idx, threshold in enumerate(thresholds, 1):
            ...
            false_pos_rate = (
                (total_extra / (total_correct + total_extra) * 100) if (total_correct + total_extra) > 0 else 0
            )

            # Calculate F1 score (harmonic mean of precision and recall)
            precision = (
                (total_correct / (total_correct + total_extra) * 100) if (total_correct + total_extra) > 0 else 0
            )
