...
def print_metadata_header(metadata: Dict[str, Any], trajectory: Dict[str, Any]) -> None:
    ...
    wrapped = textwrap.fill(citation, width=80, initial_indent="Citations:   1) ", subsequent_indent="                ")
    print(wrapped)
    if xyzg_citation:
        wrapped = textwrap.fill(
            xyzg_citation, width=80, initial_indent="             2) ", subsequent_indent="                "
        )
        print(wrapped)
    ...
    if metadata["parameters"]:
        ...
        wrapped_params = textwrap.fill(
            params_str, width=64, initial_indent="Parameters:     ", subsequent_indent="                "
        )
...
def print_analysis_results(results: Dict[str, Any], show_all: bool = False, mode: int = 0) -> None:
    """
    ...
    if results.get("displacement_files"):
        ...
        print(f"\n✓ Saved displacement structures: {os.path.basename(f_path)}, {os.path.basename(r_path)}")
...
def print_mode_characterization(characterization: Dict[str, Any], vib_results: Dict[str, Any]) -> None:
    """
...
def print_vibrational_results(results: Dict[str, Any], show_all: bool = False) -> None:
    ...
    def print_coordinate_section(title: str, data_dict: Dict, coord_type: str, unit: str) -> None:
        """
    ...
    if vib["dihedral_changes"] and (vib["bond_changes"] or vib["angle_changes"]):
        print("\nNote: Dihedrals may be artifacts of motion in the TS, not directly dependent on bond/angle changes.")
...
def print_graph_analysis(
    results: Dict[str, Any], atom_index_map: Optional[Dict[int, str]] = None, debug: bool = False
) -> None:
    """
    ...
    if "ascii_ts" in results:
        ...
        logger.debug("No ASCII data available. Run with -d or ensure generate_ascii_summary() is called.")
